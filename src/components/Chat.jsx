...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        const usersId = posts.data.map(post => auth.user.following.find(friend => friend.to_user._id === post.user._id) ? null : post.user._id);
        const uniqueUsersId = [...new Set(usersId)];

        const userName = posts.data.map(post => auth.user.following.find(friend => friend.to_user._id === post.user._id) ? null : post.user.name);
    ...
    return (
        <>
            {
                isDirectMessageOpen ? <DirectMessage setIsDirectMessageOpen={setIsDirectMessageOpen} user={clickedUser} chatRoom={chatRoom}/>
                    :
                    <div className={styles.chatContainer}>
                        <div className={styles.header}>
                            ...
                            <div className={styles.searchBar}>
                                <input className={styles.searchInput} placeholder='Search friends' type='text' />
                        ...
                        <div className={styles.friendsList}>
                                ...
                                {
                                    chatFriends && chatFriends.map((friend, index) => {
                                        return (
                                            <div key={index} onClick={() => handleFriendClick(friend)} className={`animate__animated animate__fadeIn ${styles.friend}`}>
                                                <img src={friend.to_user.avatar ? env.file_url + friend.to_user.avatar : dummyImg} />
                                                <div className={styles.friendInfo}>
                                                    <p className={styles.friendName}>{friend.to_user.name}</p>
                                                    <div className={styles.activity}>
                                                        <FontAwesomeIcon icon={faCircle} 
                                                            className={friend.status === 'Active now' ? styles.circleGreen : styles.circleGray} />
                                                        <div className={`${styles.status} animate__animated animate__fadeIn`}>
                                                            {friend.status === 'Active now' ? 'Active now' : friend.moment ? 'Active ' + friend.moment : 'Offline'}
                                                        </div>
                                        ...
                                        <div className={styles.noFriend}>
                                            ...
                                            <div className={styles.recommendations}>

                                                {
                                                    ...
                                                    users.map((user) => {
                                                        return user.avatar && user.id && user.id !== auth.user._id && (
                                                            <Link to={`/users/profile/${user.id}`} key={user.id} className={styles.recommendation}>
                                                                <img src={user.avatar ? env.file_url + user.avatar : dummyImg} />
                                                                <p className={styles.recommendationName}>{user.name.split(' ')[0]}</p>
                                                            </Link>
                                                        )
                                                    })
...

                                {
                                    auth.user.following.length === 0 && (
                                        <div className={styles.noFriend}>
                                            <p className={styles.noFriendText}>Follow someone to start Chat</p>
                                            <p className={styles.noFriendText}>Here are few recommendations</p>
